...
const schema = yup.object({
    ...
    make: yup.string().required().label('Marca'),
    model: yup.string().required().label('Modelo'),
    serial: yup.string().required().label('Serial'),
    type_device_id: yup.string().required().label('Tipo de Dispositivo'),
...
const getData = () => {
    axios
        .get(`${baseURL}/devices/${router.currentRoute.value.params.id}/edit`)
        .then((response) => {
            ...
            model.value = retriviedData.value.model;
            type_device_id.value = retriviedData.value.type_device_id;
            ...
            <form @submit="onSubmit">
                <div class="col-12 md:col-12">
                    <div class="card p-fluid">
                        <h5>Formulário Edição de Dispositivos</h5>
                        ...
                        <div class="field">
                            ...
                            <Dropdown v-model="device_status_id" :options="devicestatus" optionLabel="name" optionValue="id" placeholder="Selecionar" :class="{ 'p-invalid': errors.device_status_id }" :disabled="retriviedData.employeeholding" />
